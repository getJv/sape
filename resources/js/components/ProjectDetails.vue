<template>
  <div v-if="projectFields">
    <v-data-table
      disable-pagination
      hide-default-footer
      hide-default-header
      :headers="headers"
      :items="activeProjectFields"
      sort-by="name"
    >
                  ...
                  <v-row v-if="editedIndex < 0" justify="center" align="center">
                    ...
                    <v-col cols="12">
                      ...
                      >
                        ...
                        >
                          <BtnProjectFieldUpDown
                            :item="item"
                            :lastOrderItem="projectFields.data[projectFields.data.length -1].data.attributes.order"
                          />
                          <v-icon
                            v-if="
                  ...
                  <v-row v-else justify="center" align="center">
                    <v-col cols="12" sm="10">
                      ...
                      <component
                        ...
                        :items="editedItem.items ? editedItem.items.split(';'):[]"
                        :defaultOption="editedItem.value"
...
import BtnProjectFieldUpDown from "./BtnProjectFieldUpDown";
...
import EnumField from "./formInputs/SelectField";
...
export default {
  ...
  computed: {
    ...
    activeProjectFields() {
      return this.projectFields.data.filter(
    ...
    fieldsList() {
      ...
      this.fields.data.forEach(item => {
        if (item.data.attributes.active) {
  ...
  methods: {
    ...
    resetOrder() {
      if (
        confirm(
          "Ao confirmar os campos serão reordenados conforme sua ordem de criação. Esta operação não podera ser desfeita"
        )
      ) {
        this.$store.dispatch("projectFieldResetOrder", this.project.data.id);
      }
    },
    ...
    editItem(item) {
      ...
      this.editedItem.items = item.links.field.data.attributes.items;
      this.editedItem.value = item.data.attributes.value;
