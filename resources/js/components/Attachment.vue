    ...
    <div v-if="projectWorkflow && stepList.length > 0">
        ...
        >
            ...
            <template v-slot:item.action="{ item }">
                <ModalPdf
                    v-if="item.data.attributes.mime_type == 'application/pdf'"
                    :pdfUrl="`api/attachments/download/${item.data.id}`"
                    :pdfName="item.data.attributes.original_filename"
                />
                <v-icon color="red" class="ml-4" small @click="deleteItem(item)"
                    >mdi-delete</v-icon
                >
            </template>
 ...
 */
        save() {
            ...
            var status = this.projectStatuses.data.find(item => {
                if (
                    item.data.attributes.name == this.editedItem.workflow_step
                ) {
                    return item;
                }
            });
